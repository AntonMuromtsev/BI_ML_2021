...
def find_best_params(model, parameters, X_train, y_train, min_loss, scoring, n_folds, initial_socre =0):
    ...
    y_prediction = [0 if i < 0.5 else 1 for i in y_prediction]
    new_score = scoring._score_func(y_prediction, y_train) # calculate new metric_value
    if new_score-initial_socre > min_loss:
        ...
        if len(new_param_dict)>0:
            find_best_params(model, new_param_dict, X_train, y_train, min_loss, scoring, n_folds, initial_socre = new_score)
...
def fit_parameters(initial_model, initial_params_dict, X_train, y_train, min_loss, scoring, n_folds=5):
    ...
    for params_dict in domain_params_dicts:
        ...
        best_params = find_best_params(model, params, X_train, y_train, min_loss, scoring, n_folds).best_params_
